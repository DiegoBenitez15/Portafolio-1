...
int adivinar_palabra(char palabra[],char palabra_adivinada[],int vidas,char letra)
{
    ...
    for(i = 0;i < 20;i++)
    {
        ...
        else if(palabra[i] == '\0') /*Para deterner la funcion cuando se acabe la palabra usando el '\0' que identifica el fin de una cadena*/
        {
            break;
        }
    }
...
int comparacion(char palabras[],char adivinar[])
{
    ...
    for(i = 0;i < 20;i++)
    {
        if(toupper(palabras[i]) == toupper(adivinar[i]))
        {
        ...
        else if(toupper(palabras[i]) != toupper(adivinar[i]))
        {
            return 0;
        }
    }
